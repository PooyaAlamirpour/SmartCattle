    ...
    return {
        init: function () {
            ...
            var oTable = $('#simpledatatable').dataTable({
                "sDom": "Tflt<'row DTTTFooter'<'col-sm-6'i>124<'col-sm-6'p>>",
                "iDisplayLength": 5,
                "oTableTools": {
                    "aButtons": [
    ...
    return {
        init: function () {
            ...
            var oTable = $('#editabledatatable').dataTable({
                ...
                "sDom": "Tflt<'row DTTTFooter'<'col-sm-6'i>125<'col-sm-6'p>>",
                "oTableTools": {
                    "aButtons": [
                        "copy",
                        "print",
    ...
    return {
        init: function (input) {
            ...
            function fnFormatDetails(oTable, nTr) {
                ...
                sOut += "</tfoot>";
                sOut += "<tbody>";
                var PermissionList = obj[RowIndex].Permissions.split(",");
                for (var i = 0; i < PermissionList.length; i++) {
                    var SplitedPermissions = PermissionList[i].split("-");
                    var ActionName = SplitedPermissions[0];
                    var MethodName = SplitedPermissions[1];
                    var CommentOnPermission = SplitedPermissions[2];
                    sOut += "<tr>";
                    sOut += "<td class=''>" + ActionName + "</td>";
                    sOut += "<td class=''>" + MethodName + "</td>";
                    sOut += "<td class=''>" + CommentOnPermission + "</td>";
                    sOut += "<td class=''>";
                    sOut += "</td>";
                    sOut += "</tr>";
                }
            ...
            var oTable = $('#expandabledatatable').dataTable({
                "sDom": "Tflt<'row DTTTFooter'<'col-sm-6'i>126<'col-sm-6'p>>",
                "aoColumnDefs": [
...
var InitiateNonExpandableDataTable = function () {
    return {
        init: function () {
            /* Formatting function for row details */
            function fnFormatDetails(oTable, nTr) {
             ...
             */
            var oTable = $('#expandabledatatable').dataTable({
                "sDom": "Tflt<'row DTTTFooter'<'col-sm-6'i>127<'col-sm-6'p>>",
                "aoColumnDefs": [
                    { "bSortable": false, "aTargets": [0] },
                    { "bVisible": false, "aTargets": [1] }
                ],
    ...
    return {
        init: function () {
            var oTable = $('#searchable').dataTable({
                "paging": false,
                "ordering": false,
                "sDom": "Tflt<'row DTTTFooter'<'col-sm-6'i>123<'col-sm-6'p>>",
                "aaSorting": [[1, 'asc']],
                "aLengthMenu": [
                    [5, 15, 20, -1],
                    [5, 15, 20, "All"]
